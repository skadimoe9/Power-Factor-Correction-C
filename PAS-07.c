...
void introText(){ // Intro program di awal
	...
	printf("Program ini akan membantu anda untuk memberikan nilai dari komponen reaktif \nyang diperlukan agar faktor daya mesin anda mencapai 0.9 (di atas standar PLN)\n\n");
...
void printPF(PFC *data_mesin, int *i){ 
	...
	if(data_mesin[*i].PF >= 0.9 && data_mesin[*i].PF < 1){
		...
		printf("Power Factor Correction tidak akan dilakukan untuk mesin '%s' karena Faktor Daya telah memenuhi standar minimum\n", data_mesin[*i].nama_mesin);
	}
		...
		printf("Power Factor Correction akan dilakukan untuk mesin '%s' agar Faktor Daya dapat mencapai standar minimum\n", data_mesin[*i].nama_mesin);
	}
	...
	for(i = 0; i < jumlah_mesin; i++){
		if(data_mesin[i].PF < 0.9){
			if(data_mesin[i].jenis_beban == 1){  
				printf("\nMesin ke-%d '%s' perlu ditambahkan kapasitor dengan nilai kapasitansi sebesar %lf x 10^(-4) Farad\n", i+1, data_mesin[i].nama_mesin, data_mesin[i].komponen); 
				...
				printf("\nMesin ke-%d '%s' perlu ditambahkan induktor dengan nilai induktansi sebesar %lf x 10^(-4) Henry\n", i+1, data_mesin[i].nama_mesin, data_mesin[i].komponen); 
				printf("Setelah Power Factor Correction, Faktor Daya menjadi 0.9 dengan perubahan nilai : \nDaya Aktif (P) = %.3lf Watt\nDaya Reaktif = %.3lf VAR\n\n", data_mesin[i].P_Target, data_mesin[i].Q_Target);  
			}
		}
			
		else if(data_mesin[i].PF >= 0.9 && data_mesin[i].PF <= 1){
			printf("\nMesin ke-%d '%s' tidak memerlukan Power Factor Correction, karena Faktor Daya-nya sudah memenuhi standar minimum \n(PF >= 0.9)\n\n", i+1, data_mesin[i].nama_mesin); 
		}
